...
The schema itself eventually became a specified artifact rather than an informal agreement. The Concise Data Definition Language, [RFC 8610][ref_rfc8610], gives CBOR a notation for describing what a valid message looks like, which closes the loop that ASN.1 opened by separating abstract syntax from encoding rules.
