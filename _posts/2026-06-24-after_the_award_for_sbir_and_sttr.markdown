...
The series so far has carried a company from deciding to pursue the programs all
...
The award is a contract or a grant, and either way it binds.
A contract carries the obligations of [federal acquisition][ref_far], and a grant
...
Performance runs against the work plan, and it runs through two relationships.
...
[contracting officer][ref_contracting_officer] or grants officer who holds the
...
[for convenience][ref_termination] at the government's own choice with a
settlement of the costs already incurred.
Performance, in other words, is enforced, and failing it has named consequences.

## Reporting

Reporting is a continuing condition of the award, not a courtesy.
...
[eligibility article][related_post_eligibility] described, so it is the
...
so the reports are treated as deliverables in their own right.
...
[money article][related_post_money] warned of.
...
The government checks what it paid for, and the company must be ready.
A cost-reimbursement award is subject to audit, the
[Defense Contract Audit Agency][ref_dcaa] at the [defense agencies][research_dod]
...
[grants system][research_nih], carries its own oversight, the
[single audit][ref_single_audit] that applies once a recipient's federal spending
crosses a threshold, and all of it
rests on the records the company kept, so a clean [audit trail][ref_audit_trail],
...
The award comes with rules whose violation is not a clerical matter but a hazard.
The company certifies its compliance at points through the award's life, it obeys
the [regulatory-compliance][ref_compliance] obligations the agreement imposes, and
...
[controlled-unclassified-information][ref_cui] safeguards and the
[maturity-model certification][ref_cmmc] the department now requires, a continuing
...
exposure under the [False Claims Act][ref_false_claims] the
[data-rights article][related_post_data_rights] named, and the program has seen genuine fraud and genuine enforcement, so the
obligations are not theoretical.
The consequences of a serious failure run to repayment, to suspension, and to
[debarment][ref_debarment] from federal awards entirely, which ends a company
built on them, so integrity is not a virtue the program hopes for but a condition
it enforces.

## Closing Out

An award ends with a closeout, and a clean one matters.
...
the next award.
The closeout is the last act of the award and the first impression for the next.

## Continuing Standing

Through all of it the company maintains the standing that lets it keep competing.
...
[proposal article][related_post_phase_one] said a future proposal rests on, so a
...
The running example finishes its award properly.
The small company building its [unmanned aircraft][related_post_prototyping] files
...
current agency instructions.

## Conclusion

After the award the work is to perform what was promised, to report on it, to
...
campaign that the rest of the series began.

## References

- [Reference, Audit Trail][ref_audit_trail]
- [Reference, Contracting Officer][ref_contracting_officer]
- [Reference, Controlled Unclassified Information][ref_cui]
- [Reference, Cybersecurity Maturity Model Certification][ref_cmmc]
- [Reference, Debarment][ref_debarment]
...
- [Reference, Regulatory Compliance][ref_compliance]
- [Reference, Single Audit][ref_single_audit]
- [Reference, Termination for Convenience][ref_termination]
