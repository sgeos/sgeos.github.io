...
The [eligibility article][related_post_eligibility] cleared the two gates, so the
...
Finding an opportunity means different things at the two kinds of agency the
...
[response to a request][ref_rfp] the agency wrote.
...
[funding opportunity][ref_grant] whose scope its innovation falls inside, and
...
The opportunities are published, and an applicant learns where.
The cross-agency starting point is the [official program portal][research_sbir_gov],
...
Department of Defense in the [Defense innovation portal][research_dod] and those
...
in its [grant guide][research_nih] and through the federal
[grants portal][ref_grants_gov] that civilian agencies share, and the National
Science Foundation through its [solicitation and project pitch][research_nsf].
...
agencies tracks several calendars and watches for the openings.

## The Anatomy of a Solicitation

A solicitation is a long document with a predictable structure, and reading all
...
[federal procurement][ref_procurement] that a grant does not.
...
rather than trusting the version it first downloaded.

## Reading a Topic

A topic statement repays close reading, because it says more than it seems to.
...
[technology readiness level][ref_trl], the rung of the ladder the
[orientation article][related_post_intro] described that the work should reach.
It often states or implies a [dual-use][ref_dual_use] expectation, that the
...
it wishes had been written, is the one whose proposal fits.

## The Pre-Release Window and Talking to the Agency

When and whether the company may talk to the agency is itself part of the rules,
...
competition fair.
The open agencies work the opposite way, since their [peer-review][ref_peer_review]
...
advantage many applicants leave on the table.

## Is It a Fit, and Is It Winnable

Two honest questions decide whether a found opportunity is worth pursuing.
...
[program portal][research_sbir_gov] exposes as a searchable database, so a
...
[research-institution][ref_ffrdc] partner before the proposal rather than after,
...
answering everything that brushes its field, is the strategic core of this stage.

## Reading for Compliance

The cheapest loss is the one a proposal suffers before anyone reads its merit.
...
A solicitation says how it will judge, and that is an instruction.
...
question the reviewer was not scoring.

## The Open-Agency Path in Practice

The open agencies deserve their own note, since their process front-loads a step.
The National Science Foundation requires a short [project pitch][research_nsf]
...
and the right [funding opportunity][research_nih] among many, to match its aims
...
skipping it wastes effort on proposals that were never going to fit.

## Scale and the UAV Case

The running example shows the stage in miniature.
A small company holding a technology like the
[unmanned aircraft][related_post_prototyping] of the engineering series searches
...
Several things are left to other articles.
...
is legal advice on a particular solicitation's terms.

## Conclusion

Finding a topic and reading a solicitation are the stage where a registered,
...
ready to write, which is the subject of the next article.
