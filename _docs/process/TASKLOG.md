...
| 2026-05-30 | A107 highlighting fix: A107 kept its Keleusma scripts in untagged (unhighlighted) fences and its genuine Rust host code in rust fences. Tagged the 21 Keleusma script blocks as keleusma, left the 10 Rust blocks and the REPL session untouched. Added i64 to the Rouge lexer (Keleusma repo, now on main) so the 0.1.1 integer type highlights; re-vendored the plugin. |
