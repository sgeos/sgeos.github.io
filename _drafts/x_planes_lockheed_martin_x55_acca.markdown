...
**The replacement was a composite mid and aft fuselage with the vertical tail integrated into it.** The fuselage was made as an upper and a lower half, each roughly 65 feet long, bonded to circular frames and to each other along a longitudinal seam with adhesive and ply overlays [[Advanced Composite Cargo Aircraft Proves Large Structure Practicality, CompositesWorld][ref_cw_acca]] [[Lockheed Martin X-55][ref_x55_wikipedia]]. The skins were a sandwich, with facesheets of out-of-autoclave carbon epoxy prepreg over a nonmetallic honeycomb core [[Honeycomb Structure][ref_honeycomb]] [[Prepreg][ref_prepreg]]. **That choice is not incidental to the process.** A sandwich takes its bending stiffness from the separation of its faces rather than from their thickness,

$$
D \approx \frac{E_{f}\,t_{\text{face}}\,h_{c}^{2}}{2}
$$

so a 1.5 millimetre facesheet across a 25 millimetre core is stiffer in bending, by a factor of about 208, than the same two facesheets would be laid solid against each other. **A thin facesheet is exactly what a process with one atmosphere of consolidation can make well**, and the arrangement therefore suits the method. **It also makes the method harder**, because the air under a facesheet must find its way out past a core that is trying to seal it in, which is a documented difficulty rather than a speculative one [[Air evacuation during vacuum bag only prepreg processing of honeycomb sandwich structures][research_air_evacuation_sandwich]]. The vertical tail used a conventional autoclave-cured tape. The work was done at the Advanced Development Programs facility at Air Force Plant 42 in Palmdale [[Skunk Works][ref_skunk_works]] [[United States Air Force Plant 42][ref_plant42]].
...
**A vacuum bag has one source of pressure and it is the atmosphere.** The bag is evacuated and the air outside presses the laminate against the tool [[Vacuum Bagging][ref_vacuum_bagging]]. Writing the consolidation pressure as the differential across the bag makes the whole argument of this section visible in one line.

$$
\Delta p_{\text{bag}} = p_{\text{amb}} - p_{\text{in}}
\qquad\qquad
\Delta p_{\text{autoclave}} = p_{\text{gauge}} + p_{\text{amb}} - p_{\text{in}}
$$

**The autoclave term has something the bag term does not, and it is the only difference between the two processes.** Setting the bag pressure to zero in both gives the ratio the rest of this article turns on.

$$
\frac{\Delta p_{\text{autoclave}}}{\Delta p_{\text{bag}}} = 1 + \frac{p_{\text{gauge}}}{p_{\text{amb}}}
$$

**A vacuum bag can never press harder than the weather allows, and the ceiling falls with the elevation of the factory floor.** The autoclave's ratio, by contrast, rises with elevation, because the denominator falls while the gauge pressure does not.
...
**The reading that makes the specification attainable is the absolute one**, meaning that the bag is to be pumped down to about twenty millibar absolute, which a pump does without reference to ambient. **The two readings are not interchangeable and the industry knows it**, which is why the rule of thumb that roughly one inch of mercury is lost per thousand feet of elevation is standard advice in composites shops. **That rule is the derivative of the relation above and not a separate fact.** Differentiating the hydrostatic balance at sea level gives

$$
\left.\frac{\mathrm{d}p}{\mathrm{d}h}\right|_{0} = -\frac{p_{0}\,g}{R_{\text{air}} T_{0}} = -\frac{p_{0}}{H}
\qquad\qquad
H = \frac{R_{\text{air}} T_{0}}{g}
$$

with a scale height $H$ of 8,435 metres, which works out at **1.081 inches of mercury per thousand feet**. **The shop rule is low by 8.1 percent** and is otherwise exactly right, which is a pleasant thing to be able to say about a rule of thumb.

**The physically meaningful quantity is the differential across the bag**, and that quantity is capped by ambient under either reading.
...
**Inverting that for the pressure ratio is the question a process engineer actually asks**, which is not what a given pressure achieves but what pressure a given specification demands.

$$
\frac{p_{1}}{p_{0}} = \frac{\phi_{0}}{\phi_{1}}\cdot\frac{1-\phi_{1}}{1-\phi_{0}}
$$
...
**And compression is not even the whole of what a void resists.** A void is a bubble, and its own surface holds a pressure difference across it that grows as the bubble shrinks [[Laplace Pressure][ref_laplace_pressure]].

$$
p_{\text{void}} - p_{\text{resin}} = \frac{2\gamma}{r}
$$

**That relation is the one that decides where each process runs out.** Setting the excess equal to the available consolidation pressure gives the radius below which a void cannot be closed by pressure at all.

$$
r_{\text{min}} = \frac{2\gamma}{\Delta p}
$$

With a surface tension of 0.035 newtons per metre for an uncured epoxy, **a vacuum bag at Air Force Plant 42 stops at 0.758 micrometres and a full autoclave at 0.103**. **The autoclave's real advantage is not that it squeezes harder in bulk. It is that it reaches voids 7.35 times smaller**, and the factor is the pressure ratio exactly, because the radius is inversely proportional to it. A one micrometre void sustains 10.15 pounds per square inch on its own account, which is 76 percent of everything a bag can bring to bear.

**So the pressure was never mainly compressing the voids. It was helping to remove them**, by dissolving gas into the resin, which Henry's law makes proportional to the pressure applied [[Henry's Law][ref_henry]],

$$
c = k_{H}\,p
$$

and by driving resin into places air had been. **An out-of-autoclave prepreg does the same job by architecture instead.** The tows are only partially impregnated, so dry fibre bundles form a connected network of channels through which air and volatiles can be drawn out under vacuum before the resin flows and seals them [[Effect of room-temperature out-time on tow impregnation in an out-of-autoclave prepreg][research_outtime_tow_impregnation]] [[Out-of-autoclave prepreg consolidation: Coupled air evacuation and prepreg impregnation modeling][research_coupled_evacuation]]. Flow through that network is a Darcy problem, with the volumetric flux set by the permeability, the viscosity and the pressure gradient [[Darcy's Law][ref_darcy]].

$$
q = -\frac{k}{\mu}\,\nabla p
$$

The permeability of a bed of fibres is set by their diameter and by how tightly they are packed, in the form the composites literature writes as the Kozeny-Carman relation.

$$
k = \frac{d_{f}^{2}\,\left(1 - V_{f}\right)^{3}}{\kappa\,V_{f}^{2}}
$$

For seven micrometre carbon fibre at a fibre volume fraction of 0.55 that is about $9.23 \times 10^{-13}$ square metres. Taking a linear gradient over a path of length $L$ and a porosity $\varphi$, the time for the flow to sweep that path is

$$
t_{\text{sweep}} = \frac{\mu\,\varphi\,L^{2}}{k\,\Delta p}
$$

**Working that relation was the most useful thing this article's equation pass did, because it shows that the draft was modelling the wrong fluid.** Two entirely different fluids move through those channels. **Air leaves them, and resin closes them**, and their viscosities differ by a factor of 540,541.

$$
\frac{t_{\text{resin}}}{t_{\text{air}}} = \frac{\mu_{\text{resin}}}{\mu_{\text{air}}}
$$

**The consequence is that gas escape is not the bottleneck and was never going to be.** At 13.39 pounds per square inch, air crosses ten millimetres of dry tow in 0.01 seconds, and **four hours of vacuum would clear 12.1 metres of it**. The same path takes resin 1.47 hours to fill. **The length of the path is not the constraint on a part this size**, and since the time goes as the square of the length, a permeability a hundred times worse would still clear more than a metre.

**What the process is actually racing is whether the channels are still open when the air needs them.** That is a question about the resin's viscosity history, and therefore about how long the material has been out of the freezer. **This is where the void mechanism and the out-life argument meet**, and the next section but one is where the meeting is priced.

**It is also why debulking, the practice of applying vacuum at intervals during layup rather than only at the end, matters far more without an autoclave than with one** [[Effects of debulk temperature on air evacuation during vacuum bag-only prepreg processing][research_debulk_temperature]] [[Bulk factor characterization of heated debulked autoclave and out-of-autoclave carbon fibre prepregs][research_bulk_factor]].
...
A thermoset cure is a chemical reaction whose progress is measured as the fraction of the total reaction heat already released, and that fraction is what a calorimeter reports [[Thermosetting Polymer][ref_thermosetting]].

$$
\alpha(t) = \frac{H(t)}{H_{\text{ult}}}
$$

Epoxy amine cure is autocatalytic, because the hydroxyl groups the reaction produces catalyse the reaction that produces them, which is why the rate rises before it falls.

$$
\frac{\mathrm{d}\alpha}{\mathrm{d}t} = \left(k_{1} + k_{2}\,\alpha^{m}\right)\left(1 - \alpha\right)^{n}
$$

**Both rate constants follow an Arrhenius law**, so the time to reach a given degree of cure rises exponentially as the temperature falls [[Arrhenius Equation][ref_arrhenius]] [[Thermal models for MTM45-1 and Cycom 5320 out-of-autoclave prepreg resins][research_kratz_thermal_models]].

$$
t \;=\; A_{0} \exp\!\left(\frac{E_{a}}{R\,T}\right)
$$
...
**One caveat belongs here rather than in a footnote, because it bounds what the fit means.** The datasheet's cure options are not iso-conversional. Each reaches the state that temperature can reach, and the next section shows those states are different. **The fitted figure is therefore an apparent activation energy for reaching the manufacturer's own specification** rather than for a fixed chemical conversion, and the reason to trust it anyway is that it predicts the third entry in the same table.

**The fit can be tested against a third point the datasheet also gives.** Extrapolating to 120 degrees predicts a dwell of 3.02 hours. **The datasheet recommends four.** The recommendation is 32.2 percent longer than the kinetics require, and that difference is not an error in either. **A cure schedule is not a kinetics prediction.** It carries margin, and the equation pass changed this article's account of where that margin goes.

**The draft said the margin covers laminate thickness, and for a facesheet it does not.** Conduction across a laminate takes a Fourier time set by its thickness and its through-thickness diffusivity,
...
$$
\mathrm{Nu} = C_{Nu}\,\mathrm{Re}^{0.8}\,\mathrm{Pr}^{1/3}
\qquad\Longrightarrow\qquad
\frac{h_{\text{autoclave}}}{h_{\text{oven}}} \sim \left(\frac{p_{\text{autoclave}}}{p_{\text{oven}}}\right)^{0.8}
$$

**At the same pressure ratio that governs consolidation, that is a factor of 4.93**, falling to 3.31 if the exponent is taken at the lower end of the range correlations give. **The experimental literature supports the direction if not the coefficient**, autoclave pressure, temperature and the timing of vacuum application all showing up in measured laminate quality rather than only in the schedule [[Effects of Variation in Autoclave Pressure, Temperature, and Vacuum-Application Time on Porosity and Mechanical Properties of a Carbon Fiber/Epoxy Composite][research_autoclave_pressure_vacuum_time]]. **The autoclave's pressure buys two things at once and the article had only counted one.** It closes voids and it moves heat, and a shop that gives up the first gives up the second in the same transaction.

## Why the Post-Cure Is Not a Convenience

**The datasheet requires that any part cured below 180 degrees be given a free-standing post-cure of two hours at that temperature, and the reason is not that the low-temperature cure is merely slow.** It is that the low-temperature cure cannot finish at all.

A curing thermoset's own glass transition rises as it reacts, and the DiBenedetto relation gives that rise in terms of the degree of cure [[Glass Transition][ref_glass_transition]].
...
\qquad\qquad
f = \frac{T_{\text{cure}} - T_{g0}}{T_{g\infty} - T_{g0}}
$$

| Cure temperature | Conversion it can reach | Glass transition there |
|---|---|---|
| 80 C | 0.667 | 80 C |
| 120 C | 0.824 | 120 C |
| 130 C | 0.857 | 130 C |
...
Recurring cost falls with cumulative production under a progress ratio, in the form Wright wrote down for airframes in the 1930s [[Experience Curve Effects][ref_learning_curve]] [[Military Airframe Costs: The Effects of Advanced Materials and Manufacturing Processes][research_rand_airframe_costs]].

$$
C_{i} = C_{1}\,i^{b}
\qquad\qquad
b = \frac{\ln \varrho}{\ln 2}
$$

At a progress ratio $\varrho$ of 0.90 the hundredth unit costs 0.497 of the first, and at 0.85 it costs 0.34. **A simpler assembly does not only cost less per aeroplane. It has fewer things left to learn about**, which is the second-order effect the part-count literature reports and this article does not attempt to size [[Recurring Cost Minimization of Composite Laminated Structures, Optimum Part Size as a Function of Learning][research_recurring_cost_part_size]].

**Inverting for the length of the run is what turns the argument into a number.** Asking how many aeroplanes the recurring saving must accumulate over before it repays what the demonstration cost gives

$$
n_{\text{break-even}} = \frac{C_{\text{programme}}}{\Delta C_{\text{per aircraft}}}
$$

| Assumption | Saving an aeroplane | Aeroplanes to repay the programme |
|---|---|---|
| two minutes at 100 dollars | 120,000 dollars | 409 |
| five minutes at 150 dollars | 450,000 dollars | 109 |
| ten minutes at 200 dollars | 1,200,000 dollars | 41 |

**Somewhere between forty and four hundred aeroplanes, and about a hundred on the middle assumption.** Charging the saving against a declining learning curve rather than a flat one pushes the middle case to 210 aeroplanes, because the saving itself shrinks as the assembly gets cheaper to perform.

**That is not the industrial break-even and is not offered as one.** A production programme would weigh the saving against the non-recurring cost of its own tooling, qualification and facilities rather than against what a demonstrator cost, and this article has no figure for that. **What the calculation gives is the scale of run over which a saving of this size becomes comparable to a cost of this size**, which is the question the registry entry's word `validate` raises and cannot answer on one aeroplane.

**Two hundred and seventy-nine C-17s were built before the line closed, and C-130J deliveries had passed five hundred by 2024** [[Boeing C-17 Globemaster III][ref_c17]] [[Lockheed Martin C-130J Super Hercules][ref_c130j]], so a run of that length is not a fanciful comparison for a military transport. **But the X-55 was a run of one**, and the tooling, the qualification and the first-article learning all fall on that single article. **The demonstrator is the worst possible case for the economics it was built to demonstrate**, and the arithmetic says by roughly two orders of magnitude.
...
**Inverting for the rate rather than comparing areas is what attaches a margin to the claim.** One fuselage half is half a cylinder, so its surface follows from the two dimensions the record gives.

$$
A_{\text{skin}} = \frac{\pi D L}{2}
$$

For 19.8 metres over a nine foot diameter that is 85 square metres. Laying $N$ plies over it inside the out-life demands a sustained rate of

$$
\dot{A} = \frac{N\,A_{\text{skin}}}{t_{\text{out}}\,h_{\text{day}}}
$$
...
**Inverting the same relation for the part rather than the rate gives the sharper form of the constraint.** A shop that can sustain a given rate can lay at most

$$
N_{\text{max}} = \frac{\dot{A}\,t_{\text{out}}\,h_{\text{day}}}{A_{\text{skin}}}
$$

plies on a shell of that size before the material is out of time.

| Sustained rate | Working hours a day | Plies the out-life permits |
|---|---|---|
| 2 square metres an hour | 8 | 4 |
| 5 square metres an hour | 8 | 10 |
| 5 square metres an hour | 16 | 20 |
| 10 square metres an hour | 16 | 39 |

**A ceiling of ten to twenty plies is not a comfortable margin over what a fuselage skin needs**, and it falls in direct proportion to the area of whatever the shop decides to make in one piece. **This is the number that bounds unitisation from above**, and unlike tooling or handling it cannot be bought around.
...
survey below. The reference list holds 4,976 definitions in total, being the
research records together with forty reference sources,
...
**The equation pass then changed three more things, and two of them were the article's own explanations rather than its numbers.**

**The draft blamed the datasheet's cure margin partly on laminate thickness, and the Fourier time says otherwise.** A six millimetre facesheet equilibrates in two minutes, which is 0.83 percent of a four hour dwell. **The margin is about the oven and the tool**, and putting a number on the oven showed the autoclave's pressure buying a factor of 4.93 in convective heat transfer as well as its factor of 7.35 in consolidation. **The article had been counting one of those and not the other.**

**The draft's account of gas transport was modelling the wrong fluid.** Air and resin move through the same channels with viscosities a factor of 540,541 apart, so evacuation is fast and sealing is slow, and four hours of vacuum would clear 12.1 metres of dry tow. **The length of the path was never the constraint.** What the process races is whether the channels are still open, which is a question about out-time, and the draft had treated the two as separate arguments.

**And a figure the draft pass deleted has come back with arithmetic behind it.** The conclusion had said the saving only exists on the hundredth aeroplane, which was a rhetorical number no checker had seen, and it was removed. **Dividing the programme cost by the recurring saving puts the break-even between 41 and 409 aeroplanes and at 109 on the middle assumption.** The rhetorical figure was approximately right, which is not a reason to have kept it.
...
**What is computed here and stated as such.** The pressure at Plant 42 and the consolidation pressures that follow from it. The pressure vessel scaling. The Boyle's law void arithmetic and the surface-tension threshold beneath it. The activation energy fitted to the datasheet, and the dwell comparison built on it. The vitrification ceiling. The Darcy transport times. The convective scaling. The fastener labour and the break-even run. The layup rate implied by the out-life and the ply ceiling implied by that. **None of those is a measurement of this aeroplane.** They are what the published constants imply, and where an assumption drives an answer the assumption is named beside it.

**Four assumptions carry more weight than the rest and are named here rather than left in the text.** The surface tension of the uncured resin is taken at 0.035 newtons per metre, a representative value and not a measurement of this material. The Kozeny constant, the fibre volume fraction and the two viscosities in the transport calculation are representative in the same way, and the transport conclusion survives a hundredfold error in the permeability because the length goes as its square root. **Two of the three DiBenedetto parameters are assumptions**, and the vitrification ceiling moves between 0.741 and 0.899 across the range they plausibly occupy, while the conclusion drawn from them does not move at all. **The minutes and the burdened rate in the fastener arithmetic are assumptions spanning a factor of ten**, and the break-even run spans the same factor in consequence.
...
- [Henry's Law][ref_henry]
- [Honeycomb Structure][ref_honeycomb]
- [Joe Davies Heritage Airpark][ref_joe_davies]
- [Laplace Pressure][ref_laplace_pressure]
- [Lockheed Martin C-130J Super Hercules][ref_c130j]
...
- [Thermal Diffusivity][ref_thermal_diffusivity]
...
[ref_c130j]: https://en.wikipedia.org/wiki/Lockheed_Martin_C-130J_Super_Hercules
...
[ref_laplace_pressure]: https://en.wikipedia.org/wiki/Laplace_pressure
...
[ref_nusselt]: https://en.wikipedia.org/wiki/Nusselt_number
...
[ref_thermal_diffusivity]: https://en.wikipedia.org/wiki/Thermal_diffusivity
...
- [Development of an Autoclave Cure Cycle with Cooling and Reheating Steps for Thick Thermoset Composite Laminates][research_thick_laminate_cure_cycle]
...
- [Effects of variation in autoclave pressure, temperature, and vacuum-application time on porosity and mechanical properties of a carbon fiber/epoxy composite][research_autoclave_pressure_vacuum_time]
...
- [Influence of Void Shape, Void Volume and Matrix Anisotropy on Effective Thermal Conductivity of a Three-Phase Composite][research_void_thermal_conductivity]
...
[research_autoclave_pressure_vacuum_time]: https://doi.org/10.1177/0021998311429618
...
[research_thick_laminate_cure_cycle]: https://doi.org/10.1177/002199839703102203
...
[research_void_thermal_conductivity]: https://doi.org/10.1177/002199839603000805
